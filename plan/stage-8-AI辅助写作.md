...
- [x] **Phase 1: 基础 AI 命令** — 续写/润色/翻译/摘要/扩写/缩写/自定义指令，SSE 流式输出
- [x] **Phase 2: Copilot 行内补全** — ghost text + Tab 接受 + Esc 取消 + 800ms 防抖
- [x] **Phase 3: AI 文档对话** — 多轮对话侧栏 + 文档上下文注入 + 浮窗/侧栏双模式
- [x] **AI 内联面板** — 选中文字下方弹出（输入框 + 预设命令 + 流式结果 + 操作按钮）
- [x] **深度思考模式** — 脑袋按钮开关，思考过程折叠显示
- [x] **AI 后台配置** — 管理员页面配置 Provider/Key/Model/限额，恢复默认
- [x] **AI 订阅制** — 三档套餐（普通/VIP/Max），申请审批，按月/按年，到期自动处理
- [x] **LLM Provider 抽象层** — OpenAI 兼容接口，支持 MiniMax/DeepSeek
- [x] **Markdown 渲染** — AI 回复支持代码块语法高亮 + 完整 Markdown 格式
- [x] **选中文本气泡菜单** — AI 创作 + 格式工具合并，BubbleMenu pluginKey 分离
- [x] **图片气泡菜单** — OCR 文字提取、样式、替换、删除
- [x] **工具栏溢出** — 动态检测 + "更多"按钮收纳
