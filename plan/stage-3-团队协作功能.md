...
3. 🖼️ 图片与附件同步上传服务 (Image Upload & MinIO) - 完成度 10%
   当前状态：我们在富文本编辑器（simple-editor）底层写好了一个触发 handleImageUpload 的钩子，但并没有完美接管并广播真实图片到所有终端。
   需要做啥：把图片拖拽进组件时，真正发包给专门的 NestJS files 控制器，传进你配置过的 MinIO (AWS S3) 或者服务器本地。然后返回正确的外网链接、并生成对应的 Tiptap 的 Image 节点持久化进 Yjs 二进制里被所有人即刻看到。
